/**
 * media_manager.js
 * Client-side logic and Alpine components for Media Manager module.
 */
...
window.mediaActions = {
    ...
    reloadItems(extraParams = {}) {
        ...
        const currentManagerUrl = window.managerPageUrl || "/media";
        const currentTrashUrl = window.trashPageUrl || "/trash";
        ...
        if (Object.prototype.hasOwnProperty.call(extraParams, 'includeDeleted')) {
            ...
            deleted_media = window.location.pathname === new URL(currentTrashUrl, window.location.origin).pathname;
            if (deleted_media) {
                params.set('deleted_media', 'true');
            } else {
                params.delete('deleted_media');
        ...
        const url = `${currentManagerUrl}/items/html${query ? `?${query}` : ''}`;
    ...
    goHome() {
        ...
        const currentManagerUrl = window.managerPageUrl || "/media";
        const currentTrashUrl = window.trashPageUrl || "/trash";
        window.history.pushState(null, '', deleted_media ? currentTrashUrl : currentManagerUrl);
        this.reloadItems();
    },
    showTrash(folderId) {
        const params = new URLSearchParams();
    ...
    goManager() {
        deleted_media = false;
        const currentManagerUrl = window.managerPageUrl || "/media";
        window.history.pushState(null, '', currentManagerUrl);
        this.reloadItems();
    },
    move(itemId) {
...
/* ==========================================
   Alpine Modal Component Factories
   ========================================== */

function folderMoveModalComponent() {
    return {
        isOpen: false,
        loading: false,
        itemId: null,
...
function mediaCreateModalComponent() {
    return {
        isOpen: false,
        mode: 'folder',
        folderId: null,
...
function mediaRenameModalComponent() {
    return {
        isOpen: false,
        isDisable: false,
        itemId: null,
...
// Expose component constructors globally
window.folderMoveModal = folderMoveModalComponent;
window.mediaCreateModal = mediaCreateModalComponent;
window.mediaRenameModal = mediaRenameModalComponent;

// Helper trigger methods attached to global constructors
window.folderMoveModal.open = (itemId) => window.dispatchEvent(new CustomEvent('open-move-modal', { detail: { itemId: itemId } }));
window.mediaCreateModal.open = (data) => window.dispatchEvent(new CustomEvent('open-create-modal', { detail: data }));
window.mediaRenameModal.open = (data) => window.dispatchEvent(new CustomEvent('open-rename-modal', { detail: data }));

// Register components with Alpine
function registerMediaModals() {
    if (!window.Alpine || typeof window.Alpine.data !== 'function') return;
    window.Alpine.data('folderMoveModal', folderMoveModalComponent);
    window.Alpine.data('mediaCreateModal', mediaCreateModalComponent);
    window.Alpine.data('mediaRenameModal', mediaRenameModalComponent);
}

if (window.Alpine) {
    registerMediaModals();
} else {
    document.addEventListener('alpine:init', registerMediaModals);
}