  ...
  return {
    ...
    captureSelection() {
      ...
      if (
        ...
        this.activeEditor?.contains(selection.anchorNode)
      ) {
    ...
    formatText(command, value = null) {
      ...
      if (this.activeField !== "description" && blockCommands.includes(command))
        return;
    ...
    routeToolbarClick(event) {
      ...
      const colorTrigger = event.target
        .closest(".dropdown-wrapper")
        ?.querySelector(".color-dropdown");
      const isInlineField =
        this.activeField === "title" || this.activeField === "location";
      ...
      if (colorDropdown) {
        ...
        if (swatch && grid === firstGrid) {
          ...
          this.formatText(
            "foreColor",
            isDefault ? "#000000" : swatch.style.backgroundColor,
          );
        }
        return;
      }
      ...
      try {
        ...
        const raw =
          typeof configured === "object"
            ? Object.values(configured)[0]
            : configured;
        const size = Number(raw);
        if (Number.isFinite(size) && size > 0) this.maxUploadSizeBytes = size;
      } catch (error) {
        console.warn(
          "Không thể đọc giới hạn upload, sử dụng mặc định 50MB.",
          error,
        );
      }
    },
    ...
    openEventMediaPicker() {
      window.dispatchEvent(
        new CustomEvent("editor-open-media-picker", {
    ...
    useMediaImage(media) {
      ...
      notify.toast.success(
        "Đã chọn ảnh",
        `Đang sử dụng ảnh "${media.name || "từ Media"}"`,
      );
    },
    ...
    async uploadImage(file) {
      ...
      if (!response.ok || apiResult?.success === false || statusCode >= 400) {
        ...
        if (
          normalizedMessage.includes("anh da ton tai") ||
          normalizedMessage.includes("ten da ton tai") ||
          normalizedMessage.includes("file da ton tai")
        ) {
    ...
    async insertDescriptionImage(event) {
      ...
      if (
        ...
        file.size > this.maxUploadSizeBytes
      ) {
    ...
    validateForm() {
      const errors = {};
      if (!this.plainText(this.form.name))
        errors.name = "Vui lòng nhập tên sự kiện.";
      if (
        this.form.start_at &&
        this.form.end_at &&
        new Date(this.form.start_at) >= new Date(this.form.end_at)
      ) {
    ...
    async saveEvent() {
      this.form.name = this.sanitizeInlineHtml(
        this.$refs.titleEditor?.innerHTML || this.form.name,
      );
      this.form.location = this.sanitizeInlineHtml(
        this.$refs.locationEditor?.innerHTML || this.form.location,
      );
      ...
      try {
        ...
        if (error.code === "MEDIA_DUPLICATE") {
          notify.toast.error(
            "Ảnh đã tồn tại",
            "File này đã có trong Media và không cần tải lên lại.",
          );
    ...
    async deleteEvent() {
      ...
      try {
        ...
        if (response.ok) {
          ...
          notify.toast.error(
            "Xóa thất bại",
            resData.message || resData.detail || "Không thể xóa sự kiện",
          );
        }
      } catch (error) {
