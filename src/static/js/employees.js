...
window.employeeActions = {
  ...
  async exportEmployees(fileType = "excel") {
    const loadingId = window.notify?.toast?.loading?.(
      "Đang xuất file...",
      "Vui lòng chờ",
    );
    try {
      ...
      if (response.status === 204) {
        window.notify?.toast?.info?.(
          "Thông báo",
          "Không có dữ liệu nhân viên để xuất",
        );
        return;
      }
      ...
      if (!response.ok) {
        window.notify?.toast?.error?.(
          "Lỗi",
          "Không thể xuất danh sách nhân viên",
        );
        return;
      }
      ...
      window.notify?.toast?.success?.(
        "Thành công",
        `Đã xuất file ${filename} thành công`,
      );
    } catch (e) {
      if (loadingId) window.notify?.toast?.dismiss?.(loadingId);
  ...
  return {
    ...
    async confirm() {
      ...
      try {
        let res;
        if (this.mode === "bulk") {
          res = await window.fetchHelper.delete("employees/bulk", {
        ...
        if (res.status_code === 200) {
          window.notify?.toast?.success?.(
            "Thành công",
            res.message || "Xoá nhân viên thành công",
          );
          this.close();
          window.dispatchEvent(
            new CustomEvent("emp-deleted", {
              detail: { mode: this.mode, ids: this.employeeIds },
            }),
          );
          const tableUrl = new URL(
            window.EMP_URLS.employeesTable,
  ...
  return {
    ...
    async submit() {
      ...
      if (
        ...
        !emailPattern.test(this.form.email.trim())
      ) {
...
function employeeImportModalComponent() {
  return {
    ...
    open(options = {}) {
      this.eventId =
        typeof options === "object" && options?.eventId
          ? options.eventId
    ...
    async previewFile() {
      ...
      this.loading = true;
      try {
        const res = await window.fetchHelper.post("employees/preview-import", {
          url: val,
        ...
        if (res.status_code === 200 && res.data) {
          ...
          this.requiredFields.forEach((field) => {
            ...
            const match = normalizedHeaders.find((h) =>
              aliases.some(
                (alias) =>
                  h.normalized === alias ||
                  h.normalized.includes(alias) ||
                  alias.includes(h.normalized),
              ),
            );
            if (match) {
          ...
          window.notify?.toast?.error?.(
            "Lỗi",
            res.message || "Không thể đọc file preview",
          );
        }
      } catch (e) {
        window.notify?.toast?.error?.(
          "Lỗi",
          "Không thể kết nối máy chủ để xem trước file",
        );
      } finally {
        this.loading = false;
      }
    ...
    async validateFileUrl(value, options = { silent: true }) {
      ...
      try {
        ...
        const res = await window.fetchHelper.rawGet(
          fileUrl.toString(),
          {},
          {
            requireAuth: false,
        ...
        if (!options.silent) {
          window.notify?.toast?.error?.(
            "Lỗi",
            "Link file không đúng định dạng hỗ trợ",
          );
        }
        return false;
      } catch (e) {
        this.fileValidation = {
          ...
          type: "",
          message:
            "Trình duyệt chưa đọc được file, hệ thống sẽ kiểm tra khi xem trước",
        };
        return true;
      }
    },
    async importData() {
      if (!this.columnMap.id || !this.columnMap.name) {
        window.notify?.toast?.error?.(
          "Lỗi",
          "Vui lòng map đầy đủ cột Mã số (ID) và Họ và tên",
        );
        return;
      }
      ...
      try {
        ...
        if (res.status_code === 200) {
          window.notify?.toast?.success?.(
            "Thành công",
            res.message || "Import thành công, đang xử lý...",
          );
          this.close();
          window.dispatchEvent(
            new CustomEvent("event-employees-updated", {
          ...
          setTimeout(() => {
            ...
            if (
              window.EMP_URLS?.employeesTable &&
              document.getElementById("emp-table-container")
            ) {
              const tableUrl = new URL(
                window.EMP_URLS.employeesTable,
                window.location.origin,
              ...
              const curParams = new URLSearchParams(window.location.search);
              ["page", "limit", "search", "sort_field", "is_desc"].forEach(
                (k) => {
                  if (curParams.has(k))
                    tableUrl.searchParams.set(k, curParams.get(k));
                },
  ...
  return {
    ...
    async loadItems(folderId = "", pushHistory = false) {
      ...
      try {
        ...
        container.innerHTML =
          '<p style="padding:24px;text-align:center;color:var(--text-secondary)">Không thể tải danh sách file.</p>';
      } finally {
        this.loading = false;
    ...
    syncClasses() {
      ...
      container
        ...
        .forEach((el) => {
          el.classList.toggle(
            "selected",
            !!selId && parseInt(el.dataset.id, 10) === selId,
          );
        });
    },
    confirmSelect() {
      ...
      if (!this.isAllowedExt(sel.ext)) {
        window.notify?.toast?.error?.(
          "Lỗi",
          "Chỉ cho phép file .xlsx, .xls, .csv, .json",
        );
        return;
      }
      this.closePicker();
      ...
      window.dispatchEvent(
        new CustomEvent("emp-media-picked", {
          detail: { url: sel.url, name: sel.name },
        }),
      );
    },
    formatBytes(bytes) {
    ...
    async onFileSelected(event) {
      ...
      if (!this.isAllowedExt(ext)) {
        window.notify?.toast?.error?.(
          "Lỗi",
          "Chỉ cho phép tải lên file .xlsx, .xls, .csv, .json",
        );
        event.target.value = "";
        return;
      }
...
empImportMediaPickerComponent.open = function (options = {}) {
  window.dispatchEvent(
    new CustomEvent("emp-media-picker-open", { detail: options }),
  );
};
