...
class QueueJobServices:
    ...
    async def get_jobs(
        ...
        result = await self.crud.select(QueueJobSelect).pagination_async(
    ...
    async def get_job(self, id: UUID) -> BaseResponse[QueueJobSchema]:
        result = (
            await self.crud.select(QueueJobSchema).where(QueueJob.id == id).find_one()
        )
        if not result:
            return BaseResponse.not_found(message="Không tìm thấy job")

        job = QueueJobSchema.model_validate(result, from_attributes=True)
        return BaseResponse.ok(job, message="Lấy job thành công")
